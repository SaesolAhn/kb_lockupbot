...
<request>
    ...
    <W_STD_ISSUE_TPCD value=""/>
        ...
        try:
            ...
            # Depending on structure, it's usually in a <vector><data>... 
            # But let's look for known fields in response or just dump it during dev if this fails.
            # Assuming standard KSD XML response: <data id="ds_result_cnt"><vector result="1"><data ... value="123">
            # We'll need to robustify this once we see the actual response format. 
            # For now, let's look for any leaf with the count. 
            # Or wait for the parser module to handle it?
            # Pragma: Let's extract safely.
            # Inspecting common pattern:
            # <map id="result"> ... <CALTOT_CNT value="54"/> ... </map>
            # Let's try to find CALTOT_CNT or similar.
            pass
        except Exception:
            pass
