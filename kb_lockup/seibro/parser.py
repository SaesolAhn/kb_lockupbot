...
def _parse_xml_data(xml_content: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # WebSquare usually puts list data in a vector with a specific ID
        # Based on inspection action="dutySafeBuySkedulPListEL1", the output vector is likely named similarly or "ds_result"
        # We search for any <vector> that seems to contain list data.
        
        # In KSD/WebSquare, the result vector often has id="result" or "ds_list"
        # Let's look for known fields in the children.
        # Key fields we need to map:
        # 단축코드: REP_ISIN_CD or STD_SHRT_ISIN_CD
        # 기업명: ISSCO_NM
        # ...
        
        # Find all 'vector' tags or 'data' tags inside them
        # Let's iterate over all 'vector' elements and check if they contain our keys.
...
def _extract_xml_row(data_node: ET.Element) -> Optional[Dict[str, Any]]:
    ...
    # We might need to handle different tag names if they differ.
    # We will try multiple common variations based on inspection.
    ...
    # Shares
    # Deposit shares: Maybe ORG_NRETURN_QTY (Original Non-Return) or SAFEDP_QTY
    # In the snippet, ORG_NRETURN_QTY is empty but logically it should be there or inferred.
    # Let's check multiple. 
...
def _parse_html_table(html_content: str) -> List[Dict[str, Any]]:
    ...
    # Assuming standard table with tr/td
    # First row is header
    ...
    for tr in trs:
        ...
        # Map indices based on "단축코드 기업명 주식종류 발행형태 시장구분 등록(예탁)일 등록(예탁)주식수 반환일 반환주식수 의무보유사유 총발행주식수"
        # 0: 단축코드, 1: 기업명, 2: 주식종류 ...
